...
struct Judge {
    ...
    int measure(int i, int y, int x) {
        ...
        cin >> v;
        if (v == -1) {
            cerr << "something went wrong. i=" << i << " y=" << y << " x=" << x
                 << endl;
            exit(1);
        }
        return v;
...
struct Solver {
    ...
    vector<vector<int>> create_temperature() {
        vector<vector<int>> temperature(L, vector<int>(L, round(sqrt(S))));
        // set the temperature to i * sqrt(S) for i-th position
        for (int i = 0; i < N; i++) {
            temperature[landing_pos[i].y][landing_pos[i].x] =
                i * round(sqrt(S));
        }
        return temperature;
    }
    ...
    vector<int> predict(const vector<vector<int>>& temperature) {
        ...
        for (int i_in = 0; i_in < N; i_in++) {
            ...
            // answer the position with the temperature closest to the measured
            // value
            int min_diff = 9999;
